 ...
 * @return void
 */
function travel_booking_dashboard_menu() { 
    add_theme_page(
		TRAVEL_BOOKING_THEME_NAME,
		TRAVEL_BOOKING_THEME_NAME,
		'manage_options',
		'travel-booking-dashboard',
		'travel_booking_dashboard_page'
	);
}
add_action( 'admin_menu', 'travel_booking_dashboard_menu' );
 ...
 * @return void
 */
function travel_booking_dashboard_page() { ?>
 ...
 * @return void
 */
function travel_booking_dashboard_scripts() {

    $admin_page = isset($_GET['page']) ? sanitize_text_field($_GET['page']) : null;

    if( $admin_page === 'travel-booking-dashboard' ){
        ...
        if ( file_exists( $dependencies_file_path ) ) {
            ...
            wp_enqueue_script(
                'travel-booking-react-dashboard',
            ...
            wp_set_script_translations( 'travel-booking-react-dashboard', 'travel-booking' );


            $arrayargs = [
                'ajax_url'           => esc_url( admin_url( 'admin-ajax.php' ) ),
                'blog_name'          => TRAVEL_BOOKING_THEME_NAME,
                'theme_version'      => TRAVEL_BOOKING_THEME_VERSION,
                'inactivePlugins'    => travel_booking_get_inactive_plugins(),
                'activePlugins'      => travel_booking_get_active_plugins(),
                'review'             => esc_url('https://wordpress.org/support/theme/travel-booking/reviews/'),
                'docmentation'       => esc_url('https://docs.wptravelengine.com/docs/travel-booking/?utm_source=travel_booking&utm_medium=dashboard&utm_campaign=docs'),
                'support'            => esc_url('https://wptravelengine.com/support-ticket/?utm_source=travel_booking&utm_medium=dashboard&utm_campaign=support'),
                'videotutorial'      => esc_url('https://www.youtube.com/@wptravelengine'),
                'get_pro'            => esc_url('https://wptravelengine.com/wordpress-travel-themes/travel-booking-pro/?utm_source=travel_booking&utm_medium=dashboard&utm_campaign=upgrade_to_pro'),
                'website'            => esc_url('https://wptravelengine.com/?utm_source=travel_booking&utm_medium=dashboard&utm_campaign=website_visit'),
                ...
                'layout'             => esc_url( admin_url( 'customize.php?autofocus[panel]=nav_menus' ) ),
                'general'            => esc_url( admin_url( 'customize.php?autofocus[panel]=general_settings' ) ),
                'frontpage'          => esc_url( admin_url( 'customize.php?autofocus[panel]=home_page_setting' ) ),
                'footer'             => esc_url( admin_url( 'customize.php?autofocus[section]=footer_settings' ) ),
            ];

            wp_localize_script( 'travel-booking-react-dashboard','cw_dashboard',$arrayargs );
        }
        wp_enqueue_style( 'travel-booking-react-dashboard', get_template_directory_uri() . '/build/dashboard.css' );
    }
}
add_action( 'admin_enqueue_scripts', 'travel_booking_dashboard_scripts' );
 ...
 * @return array
 */
function travel_booking_get_inactive_plugins() {
    if (!current_user_can('install_plugins') && !current_user_can('activate_plugins')) {
        return new \WP_Error( 'rest_forbidden', esc_html__( 'Sorry, you are not allowed to do that.', 'travel-booking' ), array( 'status' => 403 ) );
    }
    ...
    foreach ($inactive_plugins as $key => $plugin_data) {
        ...
        $modified_inactive_plugins[] = array(
            ...
            'url'  => travel_booking_get_activation_url($slug)
        );
    }
 ...
 */
function travel_booking_get_activation_url($plugin_slug) {
 ...
 * @return array
 */
function travel_booking_get_active_plugins() {
